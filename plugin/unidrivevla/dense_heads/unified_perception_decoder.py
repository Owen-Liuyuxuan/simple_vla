    ...
    def forward_stage1(self, feature_maps, metas):
        ...
        if 'det' in self.query_select:
            det_feat, det_anchor, temp_det_feat, temp_det_anchor, time_interval = \
                self.det_instance_bank.get(B, metas, dn_metas=None)
        if 'map' in self.query_select:
            map_feat, map_anchor, temp_map_feat, temp_map_anchor, _ = \
                self.map_instance_bank.get(B, metas, dn_metas=None)
    ...
    def post_process(self, stage2_outs):
        ...
        if hasattr(self, 'det_decoder') and self.det_decoder is not None:
            ...
            if len(det_cls_list) > 0:
                ...
                if stage2_outs.get("det_instance_id") is not None:
                    instance_id = stage2_outs["det_instance_id"]
                elif self.with_instance_id and hasattr(self, 'det_instance_bank'):
                    try:

...
# Single-namespace/name pair (Hugging Face repo id), not a POSIX path with extra segments.
_HF_REPO_ID_RE = re.compile(
    r"^[A-Za-z0-9][A-Za-z0-9_.-]*/[A-Za-z0-9][A-Za-z0-9_.-]*$"
)


def resolve_vlm_pretrained_path(pretrained_path: str) -> str:
    """Resolve *pretrained_path* to a local directory containing weights and processor files.

    Accepts either an existing directory (e.g. downloaded Qwen3-VL) or a Hugging Face
    repo id such as ``Qwen/Qwen3-VL-2B-Instruct`` (downloads via ``snapshot_download``).
    """
    if os.path.isdir(pretrained_path):
        return os.path.abspath(pretrained_path)
    if _HF_REPO_ID_RE.match(pretrained_path.strip()):
        from huggingface_hub import snapshot_download

        return snapshot_download(repo_id=pretrained_path.strip(), local_files_only=False)
    raise FileNotFoundError(
        f"VLM pretrained path not found: {pretrained_path!r}. "
        "Set environment variable VLM_PRETRAINED_PATH to a local directory with "
        "Qwen3-VL weights (and processor files), or to a Hugging Face repo id "
...
class Qwen3VLWithExpertModel(nn.Module):
    def __init__(
        ...
        pretrained_path = resolve_vlm_pretrained_path(pretrained_path)

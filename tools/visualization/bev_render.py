...
class BEVRender:
    ...
    def _draw_occ_underlay(self, occ_bgr):
        """OCC underlay; raster from ``warp_occ_semantic_lidar_grid_to_ego_bev`` """
        if occ_bgr is None:
            return
        rgb = cv2.cvtColor(occ_bgr, cv2.COLOR_BGR2RGB)
        self.axes.imshow(
            rgb,
            extent=(-self.ylim, self.ylim, -self.xlim, self.xlim),
            origin="upper",
            zorder=0,
            alpha=0.45,
    ...
    def render(
        ...
        self.draw_planning_pred(result)
        self._render_sdc_car()
        self._render_legend()
    ...
    def render_pred_only(
        self,
        result,
        index,
    ):
        """BEV prediction panel only (no GT BEV). Same overlays as second half of :meth:`render`."""
        self.reset_canvas()
        self.draw_detection_pred(result)
        self.draw_track_pred(result)
        self.draw_motion_pred(result)
    ...
    def draw_planning_pred(self, result):
        ...
        plan_trajs = result['final_planning'].cpu().float().numpy() # [T, 2]
        ## There are only one final trajectory in final_planner, otherwise we need to read trajs_3d

        self._render_traj(plan_trajs,
                            colormap='autumn', dot_size=50)

"""models/motion/target.py → samplers/motion.py

Motion/Planning target samplers.
"""
import torch
...
def get_best_reg(reg_preds, reg_target, reg_weight):
    bs, num_pred, mode, ts, d = reg_preds.shape
    reg_preds_cum = reg_preds.cumsum(dim=-2)
    reg_target_cum = reg_target.cumsum(dim=-2)
    dist = torch.linalg.norm(reg_target_cum.unsqueeze(2) - reg_preds_cum, dim=-1)
...
class MotionTarget:
    ...
    def sample(self, reg_pred, gt_reg_target, gt_reg_mask, motion_loss_cache):
...
class PlanningTarget:
    def __init__(self, ego_fut_ts, ego_fut_mode):
        self.ego_fut_ts = ego_fut_ts
        self.ego_fut_mode = ego_fut_mode

    def sample(self, cls_pred, reg_pred, gt_reg_target, gt_reg_mask, data):

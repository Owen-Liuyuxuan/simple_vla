"""models/ego/instance_bank.py → banks/ego.py

EgoInstanceBank — ego-centric feature extraction + temporal memory.
"""
import torch
import numpy as np
...
class EgoInstanceBank(torch.nn.Module):
    def __init__(
        self,
        embed_dims,
        anchor_type='nus',
        ...
        kernel_size = tuple([int(x / 2) for x in feature_map_scale]) if feature_map_scale else (1, 1)
        ...
        if anchor_type == 'nus':
            self.anchor = torch.nn.Parameter(
                torch.tensor(nus_ego_anchor, dtype=torch.float32), requires_grad=False
            )
        elif anchor_type == 'b2d':
            self.anchor = torch.nn.Parameter(
                torch.tensor(b2d_ego_anchor, dtype=torch.float32), requires_grad=False
            )

        self.num_anchor = len(self.anchor)

        if self.with_instance_feat:
            self.instance_feature = torch.nn.Parameter(
                torch.zeros([self.anchor.shape[0], self.embed_dims]),
                requires_grad=feat_grad,
            )
        else:
            self.ego_feature_encoder = torch.nn.Sequential(
                torch.nn.Conv2d(self.embed_dims, self.embed_dims, 3, stride=1, padding=1, bias=False),
                torch.nn.BatchNorm2d(self.embed_dims),
                torch.nn.Conv2d(self.embed_dims, self.embed_dims, 3, stride=2, padding=1, bias=False),
                torch.nn.BatchNorm2d(self.embed_dims),
                torch.nn.ReLU(),
        ...
        if plan_anchor is not None:
            self.plan_anchor = torch.nn.Parameter(
                torch.tensor(np.load(plan_anchor), dtype=torch.float32), requires_grad=False
            )
            self.plan_anchor_encoder = torch.nn.Sequential(
    ...
    def cache(self, instance_feature, anchor, metas=None, feature_maps=None):

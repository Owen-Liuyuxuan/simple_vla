"""models/motion/instance_queue.py → banks/motion.py

InstanceQueue — temporal memory for motion/planning queries.
"""
import copy
import torch
...
class MotionInstanceQueue(torch.nn.Module):
    """Temporal queue for motion & planning instances.

    Collects per-frame det/map/ego features and anchors, providing
    temporal context for motion planning queries.
    """
    ...
    def __init__(
        ...
        self.ego_feature_encoder = torch.nn.Sequential(
            torch.nn.Conv2d(embed_dims, embed_dims, 3, stride=1, padding=1, bias=False),
            torch.nn.BatchNorm2d(embed_dims),
            torch.nn.Conv2d(embed_dims, embed_dims, 3, stride=2, padding=1, bias=False),
            torch.nn.BatchNorm2d(embed_dims),
            torch.nn.ReLU(),
            torch.nn.AvgPool2d(kernel_size),
        )
        self.ego_anchor = torch.nn.Parameter(
    ...
    def prepare_planning(self, feature_maps, mask, batch_size):
        ...
        if self.prev_ego_status is not None:
            ...
            ego_anchor[..., -2] = prev_ego_status[..., 6]  # VY at index -2

        if self.ego_period is None:

"""models/instance_bank.py → banks/instance_bank.py

InstanceBank with topk helper — stateful temporal memory for det/map.
"""
import torch
import numpy as np
...
def topk(confidence, k, *inputs):
    """Top-k selection across batch dimension."""
...
class InstanceBank(torch.nn.Module):
    def __init__(
        ...
        self.num_anchor = min(len(anchor), num_anchor) if num_anchor is not None else len(anchor)

...
class TemporalSeparateAttention(nn.Module):
    ...
    def get_separate_attn_mask(self, attn_mask, query_list, key_list, num_anchor_cumsum, num_temp_anchor_cumsum=None):
        ...
        for query in query_list:
            ...
            q_start, q_end = num_anchor_cumsum[q_index], num_anchor_cumsum[q_index + 1]
            sep_attn_mask1 = []
            for key in key_list:
                k_index = self.query_select.index(key)
                if num_temp_anchor_cumsum is None:
                    k_start, k_end = num_anchor_cumsum[k_index], num_anchor_cumsum[k_index + 1]
                else:
                    k_start, k_end = num_temp_anchor_cumsum[k_index], num_temp_anchor_cumsum[k_index + 1]

                sep_attn_mask1.append(attn_mask[q_start:q_end, k_start:k_end])
            sep_attn_mask2.append(torch.cat(sep_attn_mask1, dim=1))
        sup_attn_mask = torch.cat(sep_attn_mask2, dim=0)
...
class InteractiveAttention(nn.Module):
    ...
    def get_distance_attn_mask(self, sep_query, sep_query_list, sep_key_list, distance_tau, kwargs):
        ...
        for points_type in points_list:
            ...
            if points_type == 'map':
                map_points = kwargs['map_anchor'].reshape(bs, kwargs['map_anchor'].size(1), -1, 2)
            if points_type == 'plan':
                plan_points = kwargs['plan_anchor'].reshape(bs, kwargs['plan_anchor'].size(1), -1, 2)
